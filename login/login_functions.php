 ...
 */
function check_login($conx, $email = '', $pass = '') {
	...
	if (empty($email)) {
		...
		$e = mysqli_real_escape_string($conx, trim($email));
	}
	...
	if (empty($pass)) {
		...
		$p = mysqli_real_escape_string($conx, trim($pass));
	}
	...
	if (empty($errors)) { // If everything's OK.
		...
		if (mysqli_num_rows($r) == 1) {
			...
			$errors[] = 'The email address and password entered do not match those in the system.';
		}

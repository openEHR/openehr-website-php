...
<div id="MainFrame">
	...
	<div id="MainArea">
		...
		<div id="TextArea">
			...
			<p>As soon as you have specified a profile (Repository &gt; Set repository), the left-hand side explorer controls
			will populate and you will initially see the explorer with the reference model (RM) class hierarchy.
			If you have selected 'Show entire ontology in explorer?' in the Options dialog, you will see all classes from all loaded RM schemas.
			This can be useful for understanding the reference models. Normally you will probably not use this option.
			At any time, you can select a class node in the explorer, and its Reference Model definition
			...
			<p>When you start the ADL Workbench, archetypes are not compiled. The easiest way to compile them is to hit F7,
			which will compile all of the archetypes in the currently selected repository in one go. This can be interrupted with Shift-ESC.
			'Compiling' means parsing, validating and flattening. Archetypes can also be compiled individually by selecting them in the explorer.
			Other options under the repository menu allow you to compile archetypes in selected subtrees. If you select a specialised archetype,
			its specialisation parents will automatically compile.</p>
			<p>If your repository has templates in it, you can compile a template in the same way. If you select a template in the Template explorer tab,
			you will see that all its dependencies compile, i.e. all archetypes implicated in its slot-filling and specialisation statements.</p>
			<p>If your repository contains legacy '.adl' archetypes, they will initially generate differential (.adls) source form archetypes, containing the 'generated' marker at the top.
			In this case, when you hit F7 or compile by another means, you will see the '(lf)' marker in the archetype explorer,
			indicating those archetypes whose primary form is a 'legacy flat' file. If you subsequently change either the .adl file or the .adls file with an editor,
			the AWB will recompile from whichever one was changed. The archetype icons in the explorer change according to their current compilation status, as shown below.</p>
			...
			<p>The compilation process performs validation on every archetype and reports errors. Errors fall into two categories: syntactic,
			denoted with codes starting with 'S', and validity errors, denoted with 'V' codes. The errors are shown both in the status tab
			and the errors tab at the bottom. The status window is shown below.</p>
			...
			<p>All archetypes have a number of header sections containing descriptive meta-data about the archetype. The Description tab shows all of these sections,
			including author details, translation information, keywords, purpose, use, misuse and so on.</p>
			...
			<p>Each node map node is shown in three possible forms. If the node is coloured, it is defined new within the current archetype.
			If it is coloured, with a yellow border, it redefines an existing node from the flat parent. If it is solid yellow/grey, it is purely inherited.</p>
			<p>You can use the radio button controls on the right to show each node in more or less detail. The 'RM visibility'
			radio button controls enable three categories of reference model attribute that have not been archetyped to be seen. These are as follows:</p>
			<ul>
			...
			<tbody>
				<tr valign="top">
					<td style="BORDER-RIGHT-WIDTH: 0px; BORDER-TOP-WIDTH: 0px; BORDER-BOTTOM-WIDTH: 0px; BORDER-LEFT-WIDTH: 0px">
					<a href="images/rm_differential.png"><img border="0" alt="RM properties, differential form" src="images/tn_rm_differential.jpg"/> </a>
					<p><em>RM properties (differential form)</em></p>
					</td>
					<td style="BORDER-RIGHT-WIDTH: 0px; BORDER-TOP-WIDTH: 0px; BORDER-BOTTOM-WIDTH: 0px; BORDER-LEFT-WIDTH: 0px; padding-left:4em;">
					<a href="images/rm_flat.png"><img border="0" alt="RM properties, flat form" src="images/tn_rm_flat.jpg"/> </a>
			...
			<h3>Path map</h3>
			<p>Archetype definitions are inherently hierarchical structures, and as a consequence a set of paths can be extracted from any archetype.
			This 'path map' is the basis for all queries written in AQL or a-path. Every node in an archetype is guaranteed to have a unique path.
			If you select the path map in the differential and flat forms, you will see the larger number of paths being extracted from the latter.</p>
			<table style="BACKGROUND-COLOR: #ffffff" border="0" cellspacing="2" cellpadding="0" width="70%">
			<tbody>
				<tr valign="top">
					<td style="BORDER-RIGHT-WIDTH: 0px; BORDER-TOP-WIDTH: 0px; BORDER-BOTTOM-WIDTH: 0px; BORDER-LEFT-WIDTH: 0px">
			...
			<p>Paths are crucial to manipulating archetypes at runtime, and also to building queries.
			The path syntax is a slightly reduced form of XPath syntax, and can be converted to standard XPath for XML-based processing.</p>
			<p>For specialised archetypes, the Path Map under the differential view shows only paths in structures introduced in the specialised archetype,
			while the path map in the flat view shows paths due to all inherited nodes as well.</p>
			...
			<p>Paths can be selected and saved to the clipboard for use in other tools, by selecting rows (including multiple rows, by using the Ctrl key)
			and then using Ctrl+C (copy) to copy to the clipboard. The clipboard contents can be viewed from the Edit menu.</p>
          
			<h3>Slot map</h3>
			<p>Some archetypes contain slots, which are joining points to other archetypes.
			A slot is defined as a constraint that specifies the possible archetypes that may be used at this point.
			We can think of the archetypes that could fill the slot as 'suppliers', i.e. archetypes that this archetype uses,
			and archetypes having slots which the current archetype matches as 'clients'.
			The ADL Workbench evaluates the slots and displays both of these lists, as shown below.</p>
			<p><a href="images/slots_1.png"><img border="0" src="images/tn_slots_1.jpg" width="180" height="200"/> </a></p>
			
			<h3>Terminology</h3>
			<p>All archetypes contain an internal terminology, consisting of one or more 'at-codes'.
			They may also include constraint codes ('ac-codes'), and bindings between both of these and external terminologies and other ontology resources.
			These elements are found in the 'ontology' section of the archetype, such as shown here.</p>

...
<!-- ------------------------------------------- Content starts here ------------------------------------------------- -->
			...
			<ul>
				...
				<li>Secondly, it introduces the concept of self-modifying workflow in the context of health care planning, whereby explicit tasks in the goal-based care plan are devoted to assessing and modifying downstream workflow.</li>
			...
			<p>This cyclic flow of information and knowledge between care, research and education must be facilitated in order to achieve a safer and more efficient healthcare. An interoperable EHR framework can facilitate the sharing of information and knowledge between not only human users but also participating software systems. This is the aim of this thesis, which is built upon the research in the field of semantic interoperability, in particular the pioneering work by the openEHR Foundation.</p>
			<p>The journey of this thesis started with a template-based supplementary EHR system - Julius, which allows clinicians to define and share record structures for care and research. The formalism behind Julius is comparable to the openEHR archetype formalism but less expressive and without the backing of international standards. This finding led to an open source implementation of the openEHR design, which in turn initiated the validation and further improvements of the archetype formalism. The software components made the archetype formalism more accessible to academic and commercial projects around the world. </p>
			...
			<ul>
				...
				<p><em>Abstract:</em></p>
				<p>Background: Future-proof EHR systems must be capable of interpreting information structures for medical concepts that were not available at the build-time of the system. The two-model approach of CEN 13606/openEHR using archetypes achieves this by separating generic clinical knowledge from domain-related knowledge. The presentation of this information can either itself be generic, or require design time awareness of the domain knowledge being employed.</p>
				<p>Objective: To develop a Graphical User Interface (GUI) that would be capable of displaying previously unencountered clinical data structures in a meaningful way.</p>
				<p>Methods: Through &ldquo;reasoning by analogy&rdquo; we defined an approach for the representation and implementation of &ldquo;presentational knowledge&rdquo;. A proof-of-concept implementation was built to validate its implementability and to test for unanticipated issues.</p>
				<p>Results: A two-model approach to specifying and generating a screen representation for archetype-based information, inspired by the two-model approach of archetypes, was developed. There is a separation between software-related display knowledge and domain- related display knowledge and the toolkit is designed with the reuse of components in mind.</p>
			...
			<ul>
				...
				<p>This paper reports on an approach to convert ADL definitions to OWL and then attach rules to the semantic version of the archetypes. This allows for an automated means to reuse knowledge expressed in the form of rules which is also flexible and follows the same philosophy of sharing archetypes. </p>
				</li>
				...
				<p>The aim of the paper is to highlight the need to be flexible when integrating data from two separate models. However, the paper also stresses that the context and semantics of the data in either model should be taken into consideration at all times to increase the chances of true positives andreduce the occurrence of false negatives. </p>
				</li>
				...
				<p><em>Abstract:</em>Standardization of data is a prerequisite to achieve semantic interoperability in any domain. This is even more important in the healthcare sector where the need for exchanging health related data among professional and institutions is not an exception but the rule. Currently, there are several international organizations working on the definition of electronic health record architectures, some of them based on a dual-model approach.</p>
				<p>We present both an archetype modeling framework and LinkEHR-ED, an archetype editor and mapping tool for transforming existing electronic healthcare data which do not conform to a particular electronic healthcare record architecture into compliant electronic health records extracts. In particular, archetypes in LinkEHR-ED are formal representations of clinical concepts built on a particular reference model but enriched with mapping information to data sources which define how to extract and transform&nbsp;existing data in order to generate standardized XML documents.</p>
				</li>
			...
			<p><em>Abstract:</em></p>
			<p>Background: The currently existing Guideline Representation Models(GRM) present limitations which hinder the adoption of guideline oriented Clinical Decision Support Systems(CDSS). To overcome these limitations and allow a successful adoption of guideline oriented CDSS standardized GRMs, which can express Clinical Practice Guidelines(CPGs) as Clinical Decision Support(CDS) rules, are needed. Therefore the two standardized GRMs GELLO and GDL have emerged. Even though GDL is currently evaluated in different clinical areas further evaluation is needed to improve and verify its design.</p>
			<p>Objectives: The aim is to assess if GDL can be used to successfully represent CPGs as CDS rules in an EHR and to unveil any similarities between the specifications of GELLO and GDL.</p>
			<p>Methods: A small part of a severe sepsis and septic shock guideline was modeled using GDL which was then applied to mock patient data to validate GDL. Furthermore the specification of GELLO and GDL were compared against certain criteria to unveil any similarities.</p>
			<p>Results: Four GDL guides were produced for the detection and management of severe sepsis and septic shock. Results from the validation of GDL were in line with the mock patient data and results from the comparison of GELLO and GDL revealed two similarities.</p>
			<p>Conclusion: The validation indicates that GDL can support the criteria for modeling guidelines in the clinical area of severe sepsis and septic shock; due to limitations this finding cannot be generalized. The comparison of GELLO and GDL revealed similarities regarding the use of the OO approach for their design and the use of a local term binded to an external terminology.</p>
			<p>Keywords: Guideline representation models, GDL, GELLO, Clinical decision support systems, HL7, openEHR.</p>
			...
			<p><em>Abstract:</em> Health information systems (HIS) in their current form are rarely sustainable. In order to sustain our health information systems and with it our health systems, we need to focus on defining and maintaining sustainable Health Information System building blocks or components. These components need to be easily updatable when clinical knowledge (or anything else) changes, easily adaptable when business requirements or processes change, and easily exchangeable when technology advances. One major prerequisite for this is that we need to be able to define and measure sustainability, so that it can become one of the major business drivers in HIS development.</p>
			<p>Therefore, this paper analyses general definitions and indicators for sustainability, and analyses their applicability to HIS. We find that general &lsquo;Emergy analysis&rsquo; is one possibility to measure sustainability for HIS. Based on this, we investigate major enablers and inhibitors to sustainability in a highlevel framework consisting of four pillars: clinical, technical, socio-technical, and political/business.</p>
			...
			<p><em>Abstract:</em> The openEHR foundation has developed an innovative design for interoperable and future-proof Electronic Health Record (EHR) systems based on a dual model approach with a stable reference information model complemented by archetypes for specific clinical purposes.</p>
			<p>A team from Sweden has implemented all the stable specifications in the Java programming language and donated the source code to the openEHR foundation. It was adopted as the openEHR Java Reference Implementation in March 2005 and released under open source licenses. This encourages early EHR implementation projects around the world and a number of groups have already started to use this code.</p>
			...
			<p><em>Abstract:</em> Semantic interoperability should not only cover system interpretation of incoming information, but should be extended to include screen representation. This article describes a two-model approach to generate a screen representation for archetype-based information, which is inspired by the two-model approach used by openEHR for their archetypes. It provides a separation between software-related display knowledge and domain-related display knowledge and is designed with reuse of components in mind. </p>

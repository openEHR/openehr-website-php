...
<!-- ------------------------------------------- Content starts here ------------------------------------------------- -->
			...
			<p>Trabalho permanente ocorre no openEHR e dentro da Conectando Reino Unido NHS para Saúde a fim de definir abordagens técnicas que vincule arquétipos e modelos openEHR com SNOMED CT. Este trabalho destina-se a resolver uma série de desafios, incluindo como determinar o conjunto correto de atributos ao criar pontos de dados codificados em um arquétipo e como mapear uma expressão de código de pós-coordenada a uma série de pontos de dados em um arquétipo.</p>
			<p>O subconjunto de linguagem terminológica criada pela Ocean Informatics para o servidor terminologia está sendo oferecido a IHTSDO como base para uma linguagem aberta usada para definir subconjuntos dinâmicos. openEHR adotará o resultado desta colaboração com IHTSDO como subconjunto de linguagem terminologia, o que permitirá a criação de subconjuntos reutilizáveis de SNOMED CT e outras terminologias.</p>

			<h3>Interfaces Virtual EHR (vEHR) e EHR Service</h3>
			<p>Várias definições de interface são disponibilizadas pela indústria, porém duas interfaces que contemplam EHR se destacam. A API vEHR é uma interface virtual que pode ser usado em um componente de middleware oferecendo uma interface de programação integrada (IDE) para serviços de back-end de uma aplicação clínica. Ele fornece acesso ao EHR, demografia, arquétipos, templates, terminologia, consultas e contempla segurança de identidade.

</p>
			<p>O EHR Service Interface fornece uma interface de alta granularidade a um serviço de EHR, permitindo o acesso a composições EHR e outros objetos EHR de alto nível, bem como a consulta.</p>

			<h3>Suporte a vias de cuidados</h3>
			<p>Com muitas especificações e bases instituídas e estáveis em openEHR, um novo trabalho irá se concentrar no apoio a vias de cuidados distribuídos. Isto irá fornecer uma maneira padronizada de encontrar e fundir ordens distribuídas de medicamentos e status, bem como permitir a geração de um mapa de referências, admissões, discartes (dispensação) e outros eventos de transferência.</p>

			<h3>Consulta e recuperação de dados</h3>
			<p>Há uma crescente ênfase na consulta e recuperação de dados. openEHR foi projetado desde o início para garantir que os dados sejam pesquisáveis com base em um caminho de arquétipos. Caminho de arquétipos atua como identificadores confiáveis para valores de dados, além de fornecer suporte à implementação direta, por ser compatível com XPath. Uma nova linguagem de consulta, linguagem de consulta arquetipada (do inglés, Archetype Query Language - AQL) já foi <a title="Publications on Archetypes" href="../resources/publications">descrita</a> em Medinfo 2007 (sob o trabalho intitulado EQL) , e já está em uso extensivo em uma implementação openEHR. O desenvolvimento está previsto, a fim de adicionar a capacidade de consulta à base do subconjunto terminológico, de modo a oferecer suporte a consultas adequadas e inferenciais.
</p>

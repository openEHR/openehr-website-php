...
$PageName = 'Noticias de la Comunidad';
...
<!-- - - - - - - - - - - - - - - - - - - - - - - Content starts here - - - - - - - - - - - - - - - - - - - - - - - - - -->
			...
			if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) {
				...
				$q = "SELECT title, text, DATE_FORMAT(date, '%M %d, %Y') AS dr, resources, coordinates, first_name, surname FROM news_items, users WHERE category='community_news' AND news_items.item_id=$news_id AND news_items.user_id=users.user_id";		
				...
				if (mysqli_num_rows($r) === 1) { //Valid id for one item
					...
					echo "\n\t\t\t</div>";
					echo "\n\t\t\t" . '<p><a href="community_news">>> Back to Community News</a></p>';
				}
				...
				$q = "SELECT item_id, title, summary, DATE_FORMAT(date, '%M %d, %Y') AS dr FROM news_items WHERE category='community_news' ORDER BY date DESC";
				$r = @mysqli_query ($conx, $q);
				...
				if ($num > 0) { //everything went ok, display results
					...
					while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
						echo '<a href="community_news.php?id=' . $row['item_id'] . '">'.$row['title'].'</a><br/><h6>'
							. $row['dr'] . '</h6>
							<p>' . $row['summary'] . '</p><br/>';
					}

	...
	<div id="MainArea">
		...
		<div id="TextArea">
			...
			if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) {
				...
				$q = "SELECT title, text, DATE_FORMAT(date, '%M %d, %Y') AS dr, resources, first_name, surname FROM news_items, users WHERE category='industry_news' AND news_items.item_id=$news_id AND news_items.user_id=users.user_id";		
				...
				if (mysqli_num_rows($r) === 1) { //Valid id for one item
					...
					echo '<br/><p><a href="../industry_news">>> Back to Industry News</a></p>';
				}
